...
int validar(int tablero[], int etapa){
    for(int i = 0; i < etapa; i++){
	//si la fila es igual a la otra fila o distancia de filas es igual a distancia de columnas        
	if(tablero[i]==tablero[etapa] || dist(tablero[i],tablero[etapa]) == dist(i,etapa)){ 
...
void reinas(int tablero[], int n,int etapa){
    ...
    tablero[etapa] = -1;
    while (tablero[etapa] < n-1 && !exito){
        tablero[etapa] +=1;
        if(validar(tablero,etapa)){
            if(etapa == n-1){
                exito = 1;
            }
            else{
